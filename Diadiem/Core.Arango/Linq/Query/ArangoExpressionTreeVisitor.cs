...
namespace Core.Arango.Linq.Query
{
    internal partial class ArangoExpressionTreeVisitor : RelinqExpressionVisitor, INamedExpressionVisitor
    {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression expression)
        {
            ...
            var noParenthesis = MethodsWithNoParenthesis.TryGetValue(methodName, out string argumentSeprator) || prop;
        ...
        protected internal override Expression VisitQuerySourceReference(QuerySourceReferenceExpression expression)
        {
            ...
            if (expression.ReferencedQuerySource is MainFromClause mainFromClause && mainFromClause.FromExpression.Type.Name == "IGrouping`2")
            {
        ...
        protected override Expression VisitBinary(BinaryExpression expression)
        {
            ...
            ModelVisitor.QueryText.Append(" ( ");
            _ = Visit(expression.Left);

            ModelVisitor.QueryText.Append(expressionTypes[expression.NodeType]);
            _ = Visit(expression.Right);
            ModelVisitor.QueryText.Append(" ) ");
        ...
        {
            if (expression.Expression is MemberExpression member && member.Expression.Type.Name == "IGrouping`2")
            {
            ...
            else if (expression.Expression.Type.Name == "IGrouping`2")
            {
                ...
                if (groupByClause.Selector is NewExpression newExpression)
                {
                    ModelVisitor.QueryText.Append(" { ");
        ...
        {
            _ = Expression.New(node.NewExpression.Constructor, node.NewExpression.Arguments);
